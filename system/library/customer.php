...
class Customer {
	...
	public  $lastlogin;
  	...
  	public function login($email, $password, $override = false) {
		if ($override) {
			$customer_query = $this->db->query("SELECT * FROM " . DB_PREFIX .
			"customer where (LOWER(email) = '" . $this->db->escape(utf8_strtolower($email)) .
			"' OR LOWER(telephone) = '" . $this->db->escape(utf8_strtolower($email)) .
			"') AND status = '1'");
		} else {
			$customer_query = $this->db->query("SELECT * FROM " . DB_PREFIX .
			"customer WHERE (LOWER(email) = '" . $this->db->escape(utf8_strtolower($email)) .
