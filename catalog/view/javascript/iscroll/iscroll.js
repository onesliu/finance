	...
	// Constructor
		iScroll = function (el, options) {
			...
			that.options = {
				...
				handleClick:true,
				// by zhizhong 修改滑动事件的相应机制,解决左右滑不能上下滑动的问题
				// 增加一个配置项pullLock，如果为true的话，就不会阻止浏览器的默认行为，如果为false的话，就会阻止浏览器的默认行为
				pullLock: true,
...
// Prototype
	iScroll.prototype = {
		...
		_start:function (e) {
			...
			// by zhizhong 修改滑动事件的相应机制,解决左右滑不能上下滑动的问题
			if (that.options.onBeforeScrollStart && !this.options.pullLock) that.options.onBeforeScrollStart.call(that, e);
...
//                        }, that.options.zoom ? 250 : 0);
					...
					// https://groups.google.com/forum/#!topic/iscroll/XL2Ny9gpKc0
						...
						that.doubleTapTimer = setTimeout(function () {
							...
							// safari 也发现这个问题，所以这里暂时不判断终端，影响未知
...
setTimeout(function(){
//        console.log("8 "+ target.tagName + " ,  isTagNameOk " + isTagNameOk);
},300);
									isTagNameOk = (isTagNameOk && target.tagName != 'BUTTON' && target.tagName != 'IMG' && target.tagName != 'LI');
//                                }
//                            }
							if (isTagNameOk) {
								ev = doc.createEvent('MouseEvents');
